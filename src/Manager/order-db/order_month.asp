...
<% pageTitle = "New Orders by by Year: " & iyear %>

<!--#INCLUDE FILE="include/mgmt_header.asp" -->

<% REM   body: %>
<FORM METHOD="POST" ACTION="order_month.asp">
...
<%
function ListRow() %>
    <TD VALIGN=TOP ALIGN=CENTER> <% = rsList("month") %>      </TD>
...
<% end function
ListSQL = "select { fn MONTHNAME(created) } month, count(order_id) num_orders, sum(total_total) total_sum, avg(total_total) total_avg, max(total_total) total_max, min(total_total) total_min from vcturbo_receipt where { fn YEAR(created) } = " & iyear & " group by { fn MONTHNAME(created) } order by 1"
%>
