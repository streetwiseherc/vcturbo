...
function OrderGetReceipt(shopper_id, order_id)
	...
	if Not(rsOrder.EOF) then
        ...
        while Not rsOrderItem.EOF
            ...
            item.[_cc_type] = rsOrder("cc_type").Value
            item.[_cc_name] = rsOrder("cc_name").Value
			item.[_product_list_price] = rsOrderItem("product_list_price").Value
			item.[_iadjust_regularprice] = rsOrderItem("iadjust_regularprice").Value
			item.[_iadjust_currentprice] = rsOrderItem("iadjust_currentprice").Value
			item.[_oadjust_adjustedprice] = rsOrderItem("oadjust_adjustedprice").Value
...
function OrderAddItem(shopper_id, sku, pfid, quantity, placed_price, attr_text, monogram, name)
    ...
    Dim nRows
    Dim rsData
	sql = "select max(item_id) value from vcturbo_basket_item where shopper_id = '" + shopper_id +"'"
	set rsData = conn.Execute(sql, nRows, adCmdText)
