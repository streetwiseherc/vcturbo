...
function UtilGetItems(mscsOrderForm, mscsShopperId)
    ...
    while Not rsOrderItem.EOF
		...
		item.pfid = rsOrderItem("pfid").Value
		item.name = rsOrderItem("name").Value
		item.list_price = rsOrderItem("placed_price").Value
		item.[_product_list_price] = item.list_price
		
		rsOrderItem.MoveNext
	wend
end function

function UtilGetShopper(mscsOrderForm, mscsShopperId)
    Dim rsShopper
    Dim sql

	REM Get the shopper data
	sql = "select name, street, city, state, zip, country, email, phone from vcturbo_shopper where shopper_id = '" & mscsShopperId & "'"
	set rsShopper = conn.Execute(sql)

	REM Set the shopper fields
	if Not(rsShopper.EOF) then
		mscsOrderForm.[_shopper_name] = rsShopper("name").Value
		mscsOrderForm.[_shopper_street] = rsShopper("street").Value
		mscsOrderForm.[_shopper_city] = rsShopper("city").Value
		mscsOrderForm.[_shopper_state] = rsShopper("state").Value
		mscsOrderForm.[_shopper_zip] = rsShopper("zip").Value
		mscsOrderForm.[_shopper_country] = rsShopper("country").Value
		mscsOrderForm.[_shopper_email] = rsShopper("email").Value
		mscsOrderForm.[_shopper_phone] = rsShopper("phone").Value
		mscsOrderForm.[_shopper_shopper_id] = mscsShopperId
	end if
end function
