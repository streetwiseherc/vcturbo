...
REM Get the shopper data into the order form
...
if errorList.Count = 0 then
    ...
    if mscsOrderForm.[_Purchase_Errors].Count > 0 or mscsOrderForm.[_Basket_Errors].Count >0 or errorLevel > 1 then
        dim errorText
    	if mscsOrderForm.[_Basket_Errors].Count > 0 then        
    		for each errorText in mscsOrderForm.[_Basket_Errors]
    			errorList.Add(errorText)
    		next
    	end if

    	if mscsOrderForm.[_Purchase_Errors].Count > 0 then
            for each errorText in mscsOrderForm.[_Purchase_Errors]
                errorList.Add(errorText)
            next
        end if
...
<%
    ...
    Dim pageRedirect
    pageRedirect = "confirmed.asp?" & mscsPage.URLShopperArgs("order_id", mscsOrderForm.order_id)
